...
| `meta_parse` | `meta::Meta::parse` + `meta::meta_agent_role` + `rename::parse_intent` + `meta::raw_seat_work_dir` (fixed slots) + `meta::resolve_seat_dir` + `meta::explicit_seat_row` + `meta::resolve_seat_target` + `meta::contained_in` (fixed slots, fixed synthetic roots, pure only — never a fuzz-selected host path) | one session meta document, including the byte-exact orchestrator-role claim and seat-dir rows in every damage shape, or one stopped-rename intent document |
