...
| `meta_parse` | `meta::Meta::parse` + `meta::meta_agent_role` + `rename::parse_intent` + `meta::raw_seat_work_dir` (fixed slots) | one session meta document, including the byte-exact orchestrator-role claim and seat-dir rows in every damage shape, or one stopped-rename intent document |
