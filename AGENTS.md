...
| Every `KillOutcome` refusal names itself on err. `ae watchdog stop` and `start` never report one as success: a refused `_watchdog` kill keeps its pidfile and the bars, any refusal (legacy panes included) settles no session fact (meta flag, unwatched seed), and the verb exits 1; the daemon's own legacy reap is the one exception that exits nothing. | `src/watchdog_glue.rs::kill_owned_pane` (`::refusal_short` owns the phrase, `::after_failed_kill` the failed-kill reading) and `src/watchdog_lifecycle.rs::stop_decision` |
