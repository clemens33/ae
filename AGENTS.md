...
| Every `KillOutcome` refusal names itself on err. `ae watchdog stop` and `start` never report one as success: a refused `_watchdog` kill keeps its pidfile and the bars, any refusal (legacy panes included) settles no session fact (meta flag, unwatched seed), and the verb exits 1; a spawn rollback or a retire never reports a refused kill as success either — the seat is kept, never removed under a pane that may live; the daemon's own legacy reap is the one exception that exits nothing. | `src/watchdog_glue.rs::kill_owned_pane` (`::refusal_short` owns the phrase, `::after_failed_kill` the failed-kill reading), `src/watchdog_lifecycle.rs::stop_decision`, and `src/spawn.rs::run_retire` with `::rollback` (`::RollbackVerdict` the verdict, `::rollback_tail` the tail) |
