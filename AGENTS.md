...
- **A session's LOOK is session-scoped, and it has three writers with one job each.**
  ...
  upgrade reassert the canonical server-global input map on ae-owned servers only. On tmux 3.5+,
...
| A session's look is SESSION- and WINDOW-scoped, never global. Three writers, one job each: a launch writes the layout, the facts and the attention SEED; a rename rewrites the layout and the facts only; the watchdog owns the verdicts every cycle and rewrites the layout ONLY on a look-stamp change. Launch and upgrade reassert one capability-canonical server-global input map on an ae-owned server because tmux cannot scope a key binding to one session. The `ae` menu range draws only the `☰`/`=` glyph as the status line's final cell and `ae-more` draws the plain `+N`; both inherit the line's dim style while quiet. The picker resolves its invoking client's current session from one client snapshot, sets or refreshes the transient `@ae_menu_open` marker there, resolves one active-window right-edge pane, and opens right-aligned above the button with `-t <pane> -x R -y S`; selection colours light the button until a row or half-interval watchdog expiry clears it. On tmux 3.5+, Left-Down sends `session`/`window` through navigation and `ae` / `ae-more` to the picker, Right-Down sends `ae` / `ae-more` to the picker and `session` to the guarded Flip menu, and both stale Up bindings are removed. On tmux 3.4, Down keeps navigation but is a no-op on those menu ranges; Left-Up opens the picker and Right-Up opens the picker or Flip menu, so each status click opens exactly once after release. Its menus are keyboard-driven and rows pick by key. `<prefix> a` (default `C-b a`) opens the picker for its invoking client. Every menu gets release protection (`display-menu -O`); tmux 3.5+ also gets mouse handling (`-M`). A picker row switches to its captured session id, then selects `@ae_main_pane` only when a build-time membership read and an execution-time session-id guard prove it belongs there. Installed picker bindings call `~/.local/bin/ae`; checkout bindings bake their state, config and server namespace plus their absolute core | `src/theme.rs` splits the sets (`layout_options` / `fact_options` / `seed_options`); `src/session_launch.rs::apply_status_bar` / `redress_status_bar` / `stamp_window` write them; `src/watchdog_daemon.rs::reconcile_look` is the only one that rewrites them, and `::publish` the only writer of the values; `src/session_tmux.rs::status_bindings_argv` owns the bindings' server boundary |
