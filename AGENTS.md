...
| A refused ownership-checked pane kill is never reported as success: every `KillOutcome` refusal names itself on err, and `ae watchdog stop`/`start` keep what they would have removed (the pidfile, the bars, the meta flag, the unwatched seed) and exit 1; the daemon's own legacy reap is the one exception that exits nothing. | `src/watchdog_glue.rs::kill_owned_pane` (`::refusal_short` owns the phrase, `::after_failed_kill` the failed-kill reading) |
