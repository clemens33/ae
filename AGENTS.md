...
- **A session's LOOK is session-scoped, and it has three writers with one job each.**
  ...
  `MouseDown1Status` sends session/window ranges through tmux navigation and `ae` / `ae-more` to
  the fleet picker; `MouseDown3Status` sends `ae` / `ae-more` to that picker and session ranges to
  the guarded Flip menu, and stale `MouseUp*Status` bindings are removed. On tmux 3.4, Down keeps
  navigation but does nothing on menu ranges, while Up opens the keyboard-driven picker or Flip
  menu after release. `<prefix> a` (default `C-b a`) opens the same picker without colliding with
...
| `@ae_spend` is watchdog-owned, session-scoped and published only on the quota cadence (`quota_every_secs`; `0` disables it too), from one `usage::observe` pass over that session priced as `ae usage` prices it. The interval it ADVERTISES is that cadence rounded up to whole verdict cycles — what the shared counter achieves, never the request — because the reader expires a fact after two of them. An observation it cannot state exactly, and `watchdog stop`, UNSET it; the picker reads an absent or stale fact as unavailable, never as a zero, and its title sum is marked inexact whenever any running session is unavailable or itself inexact | `src/watchdog_daemon.rs::quota_effective_secs` owns the advertised cadence, `::spend_fact` / `::publish_spend` write it; `src/tmux.rs::parse_picker_spend` is the only reader, fuzzed by `picker_spend`; `src/orchestrator.rs::fleet_spend` / `::money` render it |
| The percentage ae JUDGES a quota window by is derived, never raw: a reached spend cap outranks unlimited credits, which outrank the operator's declared `manual_resets` (`0`-`9`, which spread one window's usage over `1 + n`), and an absent declaration leaves the raw window. ONE function derives it and both consumers read that one value — `ae quota`'s `EFFECTIVE` column and the watchdog threshold, with the delegation guidance pointing at the column. Two labels on one config home are one account: the SMALLEST explicit count wins, because claimed headroom nobody declared would suppress a real advisory. An unusable declaration is ignored with one visible note rather than refusing a config, and ae never CONSUMES a count — a used reset is the operator's to re-declare | `src/quota.rs::derived` is the one derivation (`::effective` its rule, `::credits_label` its credit spelling, `::merge_declaration` the pessimistic reconciliation); `src/config.rs::parse_manual_resets` owns the declaration grammar |
| Account facts (credits, spend control) are SCOPE facts with their OWN provenance, never a window's: a record replaces them only when it usably reports one, stamps itself, is newer than the facts already held and is not future-skewed. A null container, a malformed field and an ordinary account-less bucket update therefore leave a proven spend cap standing, and a qualifying record is applied whole. A balance ae cannot state exactly is reported as available WITHOUT an amount, never clipped to a different number | `src/quota/codex.rs::account_update` (`::reported_credits` / `::reported_spend_control` decide usability; `CREDIT_BALANCE_MAX` bounds an exact literal) |
| A quota window belongs to the CLIENT SCOPE, not to the conversation that observed it: the watchdog's tracked state is keyed by canonical source, bucket, qualifier and window and NEVER by rollout, so N rollout owners under one config home produce ONE advisory per transition | `src/watchdog_daemon.rs::QuotaKey` + `::quota_samples_at` (newest observation per key) |
| A quota classification answers to TWO clocks, kept apart: a vendor observation is accepted only when it is genuinely newer (an older sample stays refused), while a POLICY change — the declaration or the reported account — carries no vendor clock and instead re-derives the observation already held, cancelling any pending advisory it supersedes. Direction-symmetric: declaring a reset clears a critical, withdrawing it re-arms one | `src/watchdog_daemon.rs::QuotaCarry::reconcile` (`::QuotaPolicy` is what a level was classified under) |
| A session's look is SESSION- and WINDOW-scoped, never global. Three writers, one job each: a launch writes the layout, the facts and the attention SEED; a rename rewrites the layout and the facts only; the watchdog owns the verdicts every cycle and rewrites the layout ONLY on a look-stamp change. Launch and upgrade reassert one capability-canonical server-global input map and remove stock right-click menus on an ae-owned server because tmux cannot scope a key binding to one session; the user's tmux is untouched. The `ae` menu range draws only the `≡`/`=` glyph as the status line's first cell, before the fleet strip, and `ae-more` draws the plain `+N`; both inherit the line's dim style while quiet. The picker resolves its invoking client's current session from one client snapshot, sets or refreshes the transient `@ae_menu_open` marker there, and opens bottom-left above the status line with `-x 0 -y S`; selection colours light the button until a row or half-interval watchdog expiry clears it. On tmux 3.5+, Left-Down sends `session`/`window` through navigation and `ae` / `ae-more` to the picker, Right-Down sends `ae` / `ae-more` to the picker and `session` to the guarded Flip menu, and both stale Up bindings are removed. On tmux 3.4, Down keeps navigation but is a no-op on those menu ranges; Left-Up opens the picker and Right-Up opens the picker or Flip menu, so each status click opens exactly once after release. Its menus are keyboard-driven and rows pick by key. `<prefix> a` (default `C-b a`) opens the picker for its invoking client. Every menu gets release protection (`display-menu -O`); tmux 3.5+ also gets mouse handling (`-M`). The watchdog atomically replaces the bounded versioned `@ae_agents` roster fact each cycle, carrying its own cadence, and unsets it on stop; the picker strictly parses/fuzzes it, bounds past age and future skew from that cadence, draws frozen keyless agent rows under stable-key session rows, and degrades by client height and terminal-cell width before tmux can refuse the menu. A picker row switches to its captured session id, then selects its captured lead or agent pane only when a build-time membership read and an execution-time session-id guard prove it belongs there. Installed picker bindings call `~/.local/bin/ae`; checkout bindings bake their state, config and server namespace plus their absolute core | `src/theme.rs` splits the sets (`layout_options` / `fact_options` / `seed_options`); `src/session_launch.rs::apply_status_bar` / `redress_status_bar` / `stamp_window` write them; `src/watchdog_daemon.rs::reconcile_look` is the only one that rewrites them, and `::publish` the only writer of the values; `src/session_tmux.rs::status_bindings_argv` owns the bindings' server boundary |
