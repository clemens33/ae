...
| Every tmux client whose output ae reads is UTF-8 (`-u`): tmux prints `_` for each non-ASCII character and control byte to a client it does not judge UTF-8, and outside tmux it judges that from the locale alone (#187). The human's own terminal (`focus`, `attach`) keeps its locale | `src/transport.rs::declares_utf8`, applied in `::spawn`; pinned by `tests/it/daemons.rs::a_stop_from_a_shell_without_a_utf8_locale_still_kills_the_watchdog_pane` |
