...
- **A session's LOOK is session-scoped, and it has three writers with one job each.**
  ...
  menu after release. `prefix a` opens the same picker without colliding with stock tmux's pane
  keys. The picker carries the invoking client explicitly, and a row switches to the captured
  session id before selecting its still-member lead pane. A bind cannot be session-scoped.
