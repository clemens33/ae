...
| `state done` / `mark-done` confirmation | Done is provisional: after each `idle_nudge_secs` period the watchdog demands proof, and only `done_confirmations` later `done` declarations confirm it. `done_confirmations = 0` or `idle_nudge_secs = 0` preserves immediate done. The orchestrator main seat's sweep nudge and any quota advisory or checkpoint delivery still end a provisional done immediately; #145 tracks bringing them under confirmation. |
...
| Invariant | Owner |
|---|---|
| Done confirmation is derived only from the existing event journal and the current seat incarnation. The daemon and `ae list` share one fold; raw state and JSON remain unchanged while the table qualifies provisional and lapsed done. | `src/watchdog.rs::done_progress` owns the episode; `::declaration_current` owns declaration currency |
...
done_confirmations = 2     # later done declarations required after challenges; 0 disables
