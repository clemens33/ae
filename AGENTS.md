...
| The percentage ae JUDGES a quota window by is derived, never raw: a reached spend cap outranks unlimited credits, which outrank the operator's declared `manual_resets` (`0`-`9`, which spread one window's usage over `1 + n`), and an absent declaration leaves the raw window. `ae quota`'s `EFFECTIVE` column, the watchdog thresholds and the delegation guidance all read that one derivation, and an unusable declaration is ignored with one visible note rather than refusing a config | `src/quota.rs::effective` owns the derivation and `::credits_label` its credit spelling; `src/config.rs::parse_manual_resets` owns the declaration grammar; `src/quota/codex.rs::account_state` owns the reported facts |
