...
| `@ae_spend` is watchdog-owned, session-scoped and published only on the quota cadence (`quota_every_secs`; `0` disables it too), from one `usage::observe` pass over that session priced as `ae usage` prices it. An observation it cannot state exactly, and `watchdog stop`, UNSET it; the picker reads an absent or stale fact as unavailable, never as a zero, and draws the fleet sum in its title | `src/watchdog_daemon.rs::spend_fact` / `::publish_spend` write it; `src/tmux.rs::parse_picker_spend` is the only reader, fuzzed by `picker_spend`; `src/orchestrator.rs::money` renders it |
