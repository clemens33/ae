...
| `state <working\|waiting-user\|waiting-agent\|blocked\|done> [reason]` | Declare work state; shows in `ae list`. `waiting-agent` means waiting on ANOTHER ae agent (name them in the reason; no counterparty validation or redirect — that is a separate ruling). A `spawned.<n>` seat cannot declare `waiting-user` (exit 2): its spawner owns the human question, so it declares `waiting-agent` and lets the spawner escalate. Only `done`, `waiting-user`, `waiting-agent` and `blocked` quiet the watchdog — a vendor usage limit first seen after the declaration outranks it on the bar and in `ae list` without ending it — `working` does not, but OUTSTANDING OWN WORK defers its nudge: a request you sent that the ledger has not closed, or an agent you spawned that still holds a seat, buys quiet until the deferral ceiling, and `ae list` says so on your line. Fresh `waiting-agent` claims no human; past `idle_nudge_secs * OWN_WORK_AGE_CAP` nudge periods (the same multiplier as the own-work deferral — but at `idle_nudge_secs = 0` the deferral is vacuous while the attention ceiling scales from the documented default 300 s, so 1200 s, because zero keeps the marker and only suppresses the nudge) it escalates to exactly `blocked`: attention marker, and nudging too only when the idle-nudge cadence is enabled (`idle_nudge_secs = 0` keeps the marker and suppresses the nudge). `blocked` means a concrete EXTERNAL blocker only — a dependency, a service, a human decision elsewhere, a broken host — never a wait on another agent. Which records are news against a quiet state is ONE table, `watchdog::ends_quiet`: the seat's own re-declaration and the human's chat-bridge message end every one, a reply to one of the seat's own asks or reviews also ends `waiting-agent`, any other record that is not the watchdog's ends `done` alone, and no watchdog record is news. The watchdog's challenge footprints still decide currency inside an episode (`watchdog::declaration_current`): the idle `nudge` lapses a `done`, a crossed challenge disqualifies any declaration, and for `waiting-user` every challenge and abandoned challenge does. The read surfaces escalate on that currency rule through the ONE classifier, the ceiling aged from the declaration on both surfaces, never by recomputing it. The daemon also ends a WAIT, never a `done`, on the human's input in the seat's pane: a tmux client viewing that pane gave input STRICTLY after both the declaration and the watchdog's newest delivery there that may have painted it (`watchdog::human_input_ends_wait` over `::last_watchdog_delivery`), remembered per seat across a detach; pane output, repaint and resize end nothing. The nudge that follows an ended wait NAMES it — the state, the age of the input and the fact that a client viewing the pane gave input, never that the human answered — in its text and in its journal summary (`watchdog_daemon::nudge_words`). Named residuals: stray client input (a click, scroll, status click or window switch onto the pane) ends a wait early, at most one extra nudge because the nudge re-arms the hold; a `waiting-user` outlives a seat that resumes work without re-declaring; the input memory is lost on a daemon restart; input followed by a detach between two polls is never seen; and a directory read cannot see client activity, so a wait the daemon ended that way can still read as current on the human-marker surfaces, while the pane border follows the daemon. THREE records close a request: a `reply` closes it by REQUEST ID — for an external slotless asker (`tracked::is_external`: the chat bridges and ae's own `ae:` verbs), by the target display; a `cancel` closes it by REQUEST ID but no agent helper emits one — the only production emitters are `ae reboot --digest-only` for its own handover and `ae compact` for its own stale checkpoints, each refusing any request it did not open; and a `retire` closes it by SEAT at EITHER end — the slot it was sent to or the slot that sent it, compared by routing key and never by display name, recorded in the SAME session, that session not renamed since. Retiring a worker clears the questions it asked from its target's inbox — nobody is left to read a reply. Every other request goes on deferring, including a cross-session one read in the CALLER's log, one recorded before a rename, and one whose party vanished with no `retire` at all. `mark-done [msg]` = `state done` plus the legacy `done` event |
| `state done` / `mark-done` confirmation | Done is provisional: after each `idle_nudge_secs` period the watchdog demands proof, and only `done_confirmations` later `done` declarations confirm it; `waiting-agent`/`blocked` are challenged the same way (strict same-state re-declaration, re-armed forever, never terminally confirmed; `waiting-user` never challenged). `done_confirmations = 0` or `idle_nudge_secs = 0` preserves immediate done. The orchestrator main seat's overview prompt books its own `sweep-nudge` action and, like every quota advisory or checkpoint, ends no declaration (#145); a sweep record an older core wrote as `nudge` still lapses a provisional done for one cycle. A challenge the send helper refuses BEFORE its paste — dead pane, unproven target, storage, lock, refused notice, no composer — is journaled as an `[unconfirmed]` attempt of that challenge, `; refused pre-paste: <why>` appended and the exit still 1, so both folds count it toward `undelivered_max` and the one unreachable alert fires (#148). `watchdog::challenge_named` is the ONE grammar reading a challenge summary back, fuzzed as `challenge_summary`. |
...
| **Resume fallback** | fresh start (mints `--session-id`) | fresh start | fresh start | fresh start | fresh start (mints `--session-id`) | fresh start | fresh start |
...
- **claude's folder-trust modal blocks an untrusted working copy** before any turn lands. ae names it
  and never answers it; a reseat seed fails fast on it, exit 1, with the next step. A claude launch or
  relaunch pastes no turn, so the watchdog names it there instead.
...
| Invariant | Owner |
|---|---|
| Done and wait confirmation are derived only from the existing event journal and the current seat incarnation. The daemon and `ae list` share one fold per state; raw state and JSON remain unchanged while the table qualifies provisional and lapsed declarations. | `src/watchdog.rs::done_progress` owns the done episode, `::wait_progress` the wait episode; `::declaration_current` owns declaration currency; `::ends_quiet` is the one table of what ends each quiet state; `::challenge_named` the one challenge grammar |
...
| Every tmux format uses a printable pipe separator, never a control byte — tmux 3.4 octal-escapes those. Each format literal is written out; `SLOTS_FORMAT` deliberately uses an unspaced pipe. Every `#{&&:…}`/`#{||:…}` is binary, nested, because tmux 3.4 reads only the first two operands | `src/tmux.rs` (`FIELD_SEPARATOR` is the parser delimiter) + the control-char-free test over every format constant, read tree-wide by `tests/it/floor.rs::no_tmux_format_literal_in_src_carries_a_control_character` + `tests/it/floor.rs::every_tmux_conjunction_in_src_is_binary` |
| Every tmux client whose output ae reads is UTF-8 (`-u`): tmux prints `_` for each non-ASCII character and control byte to a client it does not judge UTF-8, and outside tmux it judges that from the locale alone (#187). The human's own terminal (`focus`, `attach`) keeps its locale | `src/transport.rs::declares_utf8`, applied in `::spawn`; the rule is pinned by `::tests::only_a_tmux_leg_that_ae_reads_is_started_as_a_utf8_client`, its wiring by `::tests::spawn_hands_a_captured_tmux_leg_u_before_its_words` |
...
| `ae compact [name]` compacts the fixed seats in place, in roster order, one nonblocking run lock for the whole run and no persisted run state: no admitted seat is dispatched before it proves a fresh durable checkpoint (a reply AND a memo carrying this request's id, both with a complete caller triple), and a seat without that proof is only ever `skipped (<reason>)` — among the reasons an identity gap, and `initiating seat` for the seat running the verb, which could never answer its own checkpoint (a caller ae cannot place is never guessed to be one); a proven seat takes the guarded dispatch whose R10 call is matched into exactly `dispatched` (attempted, never proof of submission), `skipped (<reason>)` or `not dispatched (<reason>)`; a dispatched seat is then watched, `observed idle` meaning its own frame read idle twice after the dispatch and never proof of compaction, and the run exits 1 when any admitted seat was not observed idle; spawned seats are skipped untouched, every outcome advances, and the run audits each seat, each observation, plus its own start/end | `src/seatcompact_run.rs::run` is the verb, `src/seatcompact.rs` the vocabulary/gates/report/exit and `::incarnation` the one I0 reading, `src/deliver.rs::deliver_guarded` the one guarded operation |
| A seat sitting on a prompt only the HUMAN may answer is NAMED within 2 watchdog cycles and ae NEVER answers it. The detector is PURE and cannot reach a pane — it takes a captured frame and a binary name, nothing else — and it is gated on the exact binary through that tool's adapter row (`agy`, `claude`), so a renamed binary or a tool with no row reads as no prompt and degrades SAFE. The shape must hold INSIDE a fixed window measured up from the last non-blank row, never a trailing run of non-blank rows (agy's modal ends on a status line, so a run degenerates to that one line): a question row, a SELECTED option row with at least one sibling on EITHER side of it — the selection travels with the human's arrow keys — a key-hint row below them, and NO composer, that last read by the composer's own owner (`deliver::region::composer_drawn`) rather than a second copy of its fence, scoped to the same window. Every question row is tried top-down and the first COMPLETE shape wins, because a single-shot match pairs a transcript question with the real hint row, encloses no option, and loses the live modal beneath it. STABILITY is the latch: the streak IS the flag, reaching the bound exactly once, so an episode is named once however long it lasts — `throttle_streak`'s own rule — and the clear needs a SUCCESSFUL capture, because a failed read is an absence of evidence that would otherwise flap the latch against an untouched modal. It ranks BELOW dead, the sweep, a declaration and the two vendor verdicts, and ABOVE the harness frames — a declared seat keeps its declaration on the bar, but the prompt is still booked under it and named once per episode by Notify, its record ending no declaration — because that is where `Stale` is decided and a seat waiting on a modal is silent BY NATURE — below them the feature would never draw. It reuses `Mark::NeedsYou` and the EXISTING `Reason::Blocked`, so the frozen seven-reason contract stands and no drawn format changes. A brief retry refuses while it waits, through TWO channels because daemon memory does not cross into the helper process: the daemon skips the exec from the verdict it already has, and the leg asks the pane itself, fail-closed. A turn ae PASTES waits on it too: `reseat` and `relaunch` fail fast on the same question read twice, exit 1, naming it; a session launch keeps its full wait; a claude launch, relaunch or carried reseat pastes no turn and leaves it to the watchdog | `src/watchdog.rs::human_prompt_class` is the ONE detector and `src/tool.rs::PromptSpec` each tool's shape and window; `src/watchdog_daemon.rs::book_human_prompt` owns the latch and branch 8.5, its 5c the clear; `src/events.rs::alert_meaning` maps both actions; `src/brief_retry.rs::decide` holds the refusing arm; `src/session_launch.rs::deliver_launch_turn` takes the caller's `OnHumanPrompt` |
| A usage limit — and, by the same rule on a second per-tool row, a TRANSIENT throttle — is read ONLY from the tool's own row: inside the capture's bottom window, the row begins with the tool's marker exactly (limit: claude `  ⎿`, codex `■`; transient: claude `⏺`/`●`, codex `■`) and, past spaces and NBSP, starts with a measured vendor phrase (a transient one names a 429, a 503 or an overload) — a transcript row quoting either never books it, and a tool with no measured row books neither. The latch carries its FIRST SIGHT's epoch, the instant of the episode's one `limit` record, which is booked with one Notify quoting the vendor's cell. A limit first seen no earlier than the seat's current quiet declaration outranks it — the bar, the attention marker and `ae list` read `limit`, a tie going to the human — and ENDS NOTHING: `ends_quiet` is unchanged and the release hands the bar back. One already showing when the seat declared stays under it. `ae list` agrees because a standing `limit` alert, which the journal fold keeps only while it is newer than the seat's own record, replaces the declared half of the agent's reason rather than losing the rank-3 tie to `blocked` | `src/watchdog.rs::newest_row` is the one row reader both halves share (`::limit_notice` the limit, `::error_row_for` the transient rows, `::Banner` the per-tool rows, `::bottom_window` the window cut `human_prompt_class` shares); `src/watchdog_daemon.rs::book_limit` owns the latch, the record and the Notify, and branch 6 of `::account_ordinary` the override; `src/session.rs::agent_entries` the reason |
| Auto reseat moves a seat PROVEN stuck on its vendor usage limit to the first usable declared candidate, in place, through the one reseat with `StopPolicy::Proven`: the daemon step journals holds and overdue failures and triggers at most one due seat per cycle through the send helper; the trigger journals the attempt BEFORE spawning the detached `_auto-reseat` leg, which recomputes the key, refuses a closed or mismatched attempt, re-chooses and journals ONE outcome — the first terminal outcome wins. The episode key is its first `limit`'s timestamp, which a restarted daemon re-books for the same key; every record carries it except `-done`, which names the profile left. Every writer takes the seat lock, without waiting, BEFORE re-reading and appending the journal. Each ending is said on the chat and told to the lead pair minus the moved seat plus the spawner, as the watchdog, ending nothing. Off is inert: no record, no lock, no leg | `src/autoreseat.rs::decide` + `::choose` + `::episode` own the decisions, `src/autoreseat/leg.rs::trigger` + `::run` the legs, `src/watchdog_daemon.rs::auto_reseat` + `::auto_acts` + `::auto_record` the daemon step; the records are `auto-reseat` + `-held` + `-done` + `-refused` + `-failed` + `-notice` |
...
| `ae reseat <session> <agent> --using <profile> [--stop-unknown]` moves ONE seat to another profile IN PLACE — same slot, same pane, same name, same records — and does the FULL ROUND: a tool still RUNNING is STOPPED where it stands, then the seat moves; a tool already gone takes the byte-identical path it always took, and either way the seat passes the ONE dead proof `relaunch` makes, asked with this verb's word so both verbs refuse for the same reasons in the same order and no second liveness rule exists. A pane running a KNOWN harness other than the recorded one is refused there BY NAME, records against pane plus the repair, never as busy; that reading is diagnosis only and rewrites no record. The stop is never SILENT and never GUESSED: the frame is read by the ONE owner that already answers "is a turn running", `harness_state::classify` — never `deliver`'s busy reading, which answers whether the input BOX holds text and is FALSE mid-turn — and only a POSITIVELY proven `Idle` frame may be stopped. `Busy` refuses ALWAYS, naming `interrupt <agent>`, and no flag lifts it; `Unknown` refuses unless `--stop-unknown`, which says "stop it anyway" and never that the frame was read. The frame is read TWICE, `FRAME_GAP` apart, because between Enter and a drawn spinner the box is empty and ONE reading calls a seat just handed work idle. Only claude and codex have grammars that classifier owns: muse declares claude's input model and draws its own frame, so it reads `Unknown` and fails CLOSED, and every unmodelled composer is `Unknown` always — and even claude proves `Idle` from the pane's recent output, so a seat that has not finished a turn since it started reads `Unknown` too. The mechanism is `respawn-pane -k -c <work_dir>`, the SAME tmux verb the monitor panes already use, through the EXISTING tmux door: measured on 3.7b it keeps the pane id, the pane's `@ae_*` options, its index and its scrollback, kills the pane's process tree and leaves tmux's default shell in the directory `-c` names — no signal, no new door, no `unsafe`. Everything DURABLE is answered from the records before tmux is read at all — the argv, the session, the caller (a pane ae STAMPED may reseat only its own session; an unstamped shell may reseat any), the ROSTER seat, the profile differing from the recorded one, and the profile resolving exactly as `run::read_seat` resolves a seat with no client override (no tool-class judgement: a reseat accepts the profiles a LAUNCH accepts) — so a stopped session diagnoses a typo exactly as a running one does; the pane's own stamp is then checked against the roster slot, because a disagreement would publish for one seat and paste into another's pane. Under `.lifecycle.<session>.lock`, in this order: STOP a running tool — the working copy first, because it is a durable fact the records already carry and the shell is respawned into it, then the caller proven NOT to be running under the target (the pane check upstream is only half that rule; a process the target's own tool started carries no `$TMUX_PANE` to compare, and a gap in the pid, the process table, the pane's or the caller's own row, or the caller's ancestry REFUSES rather than kills, naming it), then the TREE from that same snapshot, where a known harness the seat does not record, or one ae cannot place outside the tree, REFUSES by tool, pid and parent because the respawn ends the whole tree, and no flag lifts it, then the pane's own SEND-LOCK so no delivery lands a turn between the frame ae read and the tool it ends (a delivery in flight is a refusal, not a wait, because the lifecycle lock is already held), then the frame, then the respawn, then a BOUNDED 10 s wait for the pane to come back to an idle shell, which on timeout refuses by name with the meta UNTOUCHED; RECORD the stop, once the pane is proven back at its shell and NEVER before, naming the binary ended because the meta keeps only the current one; prove dead; BUILD the seed, before anything is removed, because it reads the recorded first message the next step deletes; publish `seed.<agent>.md` 0600 and KEEP it, since it is what a human re-sends by hand; `run::clear_slot`; ONE guarded meta replacement; re-read the seat; paste and observe. The lock is DROPPED before any readiness wait, because a gated turn blocks up to 45 s. Past it the tool's OWN launch turn goes first where its adapter has one — codex's rollout does not exist until a user turn and that handshake must not sit under a 24 KB seed — then the seed as `⟦ae:ctx⟧`, then the capture, then a LAST identity reading, because exit 0 means the seat is up NOW. The move WRITES the profile, the tool, a fresh conversation (a UUID where the tool takes one at launch, `pending` otherwise), a launch token and a capture floor, and appends the old conversation to the predecessor list TAGGED with the tool that OWNS it, since the successor's tool reads a different store; it REMOVES the config home and its implicit base (they belong to the tool that is leaving), the launch time (it would date a launch that has not happened), the observed model and its pin (they would read as drift the moment the new tool answers) and the launch's `client` override — removed, never emptied, because an empty override is not the same fact as no override. The guard is the SEAT and not the slot: a `retire` plus a re-`spawn` can land a successor there mid-flight and it must not inherit the move. NO crash window is atomic and none needs to be: before the meta write the seat still records its old profile with no start marker — whether the stop had happened or not — after it the pane sits at a shell, and `relaunch` finishes all three. The seed is the pack `ae brief --seat` renders, from ONE builder, so what a human reads before a move and what the successor is handed are the same document — and because that document is PASTED, every field the pack takes out of a record routes through the ONE neutraliser, control bytes included: an escape sequence, a bell or a bracketed-paste terminator in a memo, a reason or a quoted turn would be KEYSTROKES in the successor's pane. ONE field is PROVEN instead of cleaned — a request id, because the pack also prints it into the `reply` command the successor is told to run and that command quotes an id without escaping it: ae mints every id it owns, so a row failing that grammar is dropped WHOLE and COUNTED, never rendered and never repaired, since a cleaned id would name a request that does not exist. BETWEEN TWO ACCOUNTS OF ONE TOOL THE CONVERSATION TRAVELS instead: the SAME recorded binary, an adapter row declaring its store a portable file set (claude only — every other row is unmeasured, not judged), a grammar-proven conversation id, and two usable config homes that DIFFER make the move COPY that store into the target account and KEEP the conversation. The bar is NO SILENT CLOBBER, NO FOLLOWED LINK and NO SILENT SKIP between two homes of the SAME OS user — never a guarantee against a writer who already owns the account, because the residual window between a classification and the read or write that follows it needs `openat`, which this crate cannot spell without the libc dependency it forbids; it is NAMED rather than closed. Copy, never move, because the source is the rollback; nothing is parsed, so no parser and no fuzz target are owed; no link is followed at ANY level, every component from the account root down classified before it is read, written or descended, each one proven a plain file name so no path can climb out of the home it names; nothing in the target is overwritten, which is a `hard_link` onto the final name and NEVER a rename — the target account is outside ae's lifecycle lock, so the one syscall that would create the name is the one that refuses it — while a byte-identical node is ae's own interrupted attempt, left untouched, mtime included, and proven again at the commit boundary; a node ae cannot even stat is a FAILURE and never an absence, a listed entry that vanished with it; and the copy set is BINDING — the transcript plus every sidecar the source has, an absent optional source being no failure. That set is walked FIRST with `carry::Mode::Check`, under the lock and BEFORE a running tool is stopped — every node classified, read and compared as the copy would, nothing created — so a read ae cannot make or a target it cannot explain REFUSES the move with nothing stopped, written or journaled; only what the Check cannot see, a write that fails or a change after it (the target account is outside the lock), abandons the whole carry LOUDLY to today's seed path, the move still happening. The TRANSCRIPT IS THE COMMIT MARKER: sidecars and project memory first, transcript last, each published through the ONE exclusive creation ae already owns — a temp under a nonce that call opened, then the link — so a crash leaves nothing findable and the next attempt converges. Project `memory/` is copied only into an account that has none and NEVER merged, a real directory there being the kept one and anything else a refusal that says what it found. Typing the move with the other account's profile IS the consent — no prompt, and ONE line names the crossing. A carried seat is handed NO seed pack and its start marker is PUT BACK after `run::clear_slot`, because `_run` reads that file to choose resume over create — and the write is PROVEN possible before anything is copied or removed, since the two files cannot be one write and the state between them is the one that would open a NEW conversation beside the store ae just copied; the ONE guarded replacement then keeps `harness_session` and `capture_floor`, writes NO predecessor row, and rewrites the config home and its base to the target as one identity. The STOP record names NO conversation — which binary was ended, in which pane, and which move it belongs to is the whole of that event, and it is durable BEFORE the carry verdict exists, so any word for a conversation there would be a guess. The MOVE record spells the conversation by what happened to it: every other arm names what the move left behind, `prior <id>`, but a carried conversation is the SAME one continuing in the other account, so the carry arm reads `conversation <id>, carried` — a durable record must not send a later reader after a live conversation believing it dead, and a seeded fallback, which really did abandon one, keeps `prior`. Everything else is byte-identical to the tool-change move, a move that was never a carry question writing exactly what it always wrote. | `src/reseat.rs::Carried` is the fate the record carries, `::field` and `::word` the only two spellings of it, and `::Mentions` decides at each `::record` call site whether a record names a conversation at all; `src/carry.rs` owns the carry (`::plan` its silent eligibility, `::run` the ordered copy, `::under` the no-link component walk every path is built through, `::publish` the exclusive publication, which is `init::create_exclusive` and no second copy of it, `::still_holds` the commit-boundary proof, `::project_key` the one owner of the store's cwd key that `rename.rs` also reads — `run::resumable` still spells it inline, guarded by `tests/it/doors.rs`); `src/meta.rs::Conversation` is the one value the two arms differ by; `src/reseat.rs` is the verb, its ladder and its order, and `::stop_running_tool` the stop with `::pane_back_at_shell` its pure idle decision; `::mixed_refusal` the tree refusal; `crate::harness_state::classify` is the ONE frame reading and `src/session_tmux.rs::Op::RespawnPane` the ONE mechanism; `src/procs.rs::lineage` is the ONE upward walk that proves the caller is not under the seat; `src/procs.rs::observed_harness` is the ONE foreground-to-tool mapping, diagnosis only (`seat_relaunch::mismatch_refusal` and `ae doctor`'s `seat-identity` rows), and `::harness_rows` its one walk; `src/seat_relaunch.rs::{prove_dead,start,observe_identity,turn_verdict,usable_work_dir}` is the borrowed operation and `::Verb` the one word that differs between the two callers; `src/meta.rs::reseated` is the pure move, `::seat_move_for` its guard and `::publish_seat_move` the only writer; `src/lib.rs::seat_pack` is the ONE pack builder `ae brief --seat` and the seed both read, and its reply lines are spelled by `src/tracked.rs::reply_command_for`, the ONE chooser the ask/review footer asks too; `src/seatpack.rs::neutralise` is the ONE neutraliser, pinned by `::nothing_a_terminal_would_act_on_survives_into_a_pack_that_is_pasted`, and `::minted_requests` the one grammar gate, pinned by `::a_request_id_ae_never_minted_is_dropped_whole_and_counted` |
| Every `KillOutcome` refusal names itself on err. `ae watchdog stop` and `start` never report one as success: a refused `_watchdog` kill keeps its pidfile and the bars, any refusal (legacy panes included) settles no session fact (meta flag, unwatched seed), and the verb exits 1; a spawn rollback or a retire never reports a refused kill as success either — the seat is kept whenever its pane's kill is refused, and a retire whose panes cannot be enumerated removes nothing; the daemon's own legacy reap is the one exception that exits nothing. | `src/watchdog_glue.rs::kill_owned_pane` (`::refusal_short` owns the phrase, `::after_failed_kill` the failed-kill reading), `src/watchdog_lifecycle.rs::stop_decision`, and `src/spawn.rs::run_retire` with `::rollback` (`::RollbackVerdict` the verdict, `::rollback_tail` the tail) |
...
done_confirmations = 2     # later done/waiting-agent/blocked declarations required after challenges; 0 disables
auto_reseat = off          # move seats off a vendor usage limit: on skips main, all moves it; orchestrator never; global only
auto_reseat_sessions = a   # sessions the path acts in; absent means every session
auto_reseat_grace_secs = 600  # limit-to-move wait; 0 moves on the first cycle past the latch

[auto_reseat]
sol6x = opus55x, spark13cm  # where sol6x may move, best first; a profile keyed twice keeps its later row
