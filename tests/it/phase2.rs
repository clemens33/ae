...
//!
//! Each test names the criterion it answers. The unit-level halves live beside their code in `src/liveness.rs`
...
impl Scratch {
    ...
    /// A session directory whose meta is absent — record loss, and a
    /// `missing` selector.
...
/// One event line, in the documented shape.
...
fn criterion_14_the_whole_phase_survives_the_filesystem_disappearing_after_inventory() {
    // The obligation is on PHASE 2, which the gate defines as the classified
    // set PLUS the successor digest.
    ...
    assert_eq!(
        ...
        "every digest fact came from the snapshot phase 1 captured"
    );
    let Some(json::Value::Arr(entries)) = document.get("sessions") else {
        panic!("sessions must be an array");
    };
...
fn criterion_16_every_successor_digest_is_version_2_with_a_closed_status_domain() {
    ...
    // ONE server response carrying every exact name any shape needs.
    let world = || {
        Recorder::new().live(
            named("B"),
            &[
    ...
    // Each shape names the statuses it must CONTAIN.
...
// ---- criterion 17: version 2 preserves the rest of the schema --------------
...
fn criterion_17_the_version_bump_changes_the_version_and_the_status_domain_only() {
    ...
    assert!(document.get("generated_at").is_some(), "field kept");
    let Some(json::Value::Arr(entries)) = document.get("sessions") else {
        panic!("sessions must be an array");
    };
    let entry = &entries[0];
    ...
    assert_eq!(
        entry.get("degraded"),
        None,
        "the omission rule for a healthy entry is unchanged"
    );
...
fn criterion_20_typed_name_and_socket_routing_reach_two_different_real_servers() {
    ...
    if !tmux_present(&scratch) {
        ...
        // environment without one has not run it.
        let _ = fs::remove_dir_all(&scratch);
        panic!(
...
fn plant_both_record_sources(scratch: &Scratch) -> PathBuf {
    ...
    // Event-derived facts the digest emits: goal_set_epoch, the declared
    // agent state, and last_active_epoch.
    scratch.events(
        "alpha",
        &[
