...
#[cfg(test)]
mod tests {
    use super::{REPLAY_RECORDS, banner, complete_len, cut_summary, format_event, replay};
    use std::fs;
    ...
    fn an_absent_or_unreadable_container_replays_nothing() {
        ...
        let missing = crate::store::open(&scratch.0).container();
        ...
        assert!(replay(&crate::store::open(&scratch.0).container()).is_empty());
    }
