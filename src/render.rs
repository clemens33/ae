    ...
    fn done_state_contract_names_confirmation_and_drops_the_legacy_claim() {
        for rendered in [RULES, MANIFEST_TEMPLATE] {
            ...
            assert!(
                ...
                "an own event no longer ends a wait: {rendered}"
            );
            assert!(
