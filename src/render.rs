...
${sessions_dir}/state waiting-user "which layout: vertical keeps panes readable | horizontal shows more context (recommend vertical because readability matters; details: .local/layout.md)"
${sessions_dir}/state blocked "blocked on codex review; reviewer unblocks req-Z"
...
Bounded subtasks go to spawned workers, then get retired. The win is CONTEXT
HYGIENE first (a worker burns its own context exploring and returns a distilled
summary — your strategic context stays clean), parallelism second, cost third.
...
doc syncs, independent review lanes. **Keep it with a lead only when** the
subtask is pure judgment: architecture, rulings, ambiguous diagnosis, final
gates, or work needing accumulated context but no tree change. **Prefer ae
...
mod tests {
    ...
    #[test]
    fn state_guidance_requires_a_self_contained_600_character_decision() {
        let dir = scratch("state-guidance");
        let document = context_document(&dir, "s", "/w", "main", &[]);
        assert!(
            document.contains("self-contained and ≤600 chars"),
            "{document}"
        );
        assert!(document.contains("never see pane"), "{document}");
        assert!(
