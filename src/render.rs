...
${sessions_dir}/state waiting-user "which layout: vertical | horizontal (recommend vertical because panes stay readable)"
...
const STATE_GUIDANCE: &str = " STATE REASONS: `waiting-user` reason is the decision question, one line ≤180 chars, shaped `<what you need decided>: <option A> | <option B> (recommend A because …)`. `blocked` reason names blocker and who/what unblocks it. Good: `which layout: vertical | horizontal (recommend vertical because panes stay readable)`. Bad: `need clarification`.\n";
