...
Everything the working contract used to carry, removed from `AGENTS.md` on 2026-09-04 so
that file could be a short, current contract instead of a record. Nothing here was
rewritten: every section below is the text as it stood at `v2026.9.2`, verbatim, and each
one is a snapshot of what was true when it was written — not a rule in force. When this
file and `AGENTS.md` disagree, `AGENTS.md` wins.

## Index

1. [Philosophy and Rules, as written before the rewrite](#1-philosophy-and-rules-as-written-before-the-rewrite) — the single-bash-script rule and the scope notes that survived it
2. [Revisit triggers](#2-revisit-triggers) — the four triggers, which fired, and the herdr watchlist
3. [Structure and doctrine docs](#3-structure-and-doctrine-docs) — the annotated tree and the three doctrine pointers
4. [How it works, end, archive and purge](#4-how-it-works-end-archive-and-purge) — launch sequence and the `--purge-history` reasoning
5. [Session helpers: generation, argv\[0\] dispatch, `_run`](#5-session-helpers-generation-argv0-dispatch-_run) — the full helper table and the symlink/write hazards
6. [Agent tool capabilities: key constraints](#6-agent-tool-capabilities-key-constraints) — readiness, capture, the agy trust modal, tmux 3.4 framing
7. [Rust era: finishing epic #79](#7-rust-era-finishing-epic-79) — the Z1–Z4 slice narrative and the status-after-each records
8. [Toolchain and lanes: the reasoning](#8-toolchain-and-lanes-the-reasoning) — why each pin is a pin, and what each lane is for
9. [Prebuilt ae distribution](#9-prebuilt-ae-distribution) — release order, published shape, install gate, `AE_NEXT_HOME` retirement
10. [Lint policy, tests and supply chain](#10-lint-policy-tests-and-supply-chain) — the lint reasoning, nextest/doctests/mutants, `deny.toml` clause by clause
11. [Dependency posture (2026-08-24)](#11-dependency-posture-2026-08-24) — the researched zero-dep line and the P4.3 risk acceptance
12. [Fresh-clone reproducibility and the musl target](#12-fresh-clone-reproducibility-and-the-musl-target) — the bootstrap contract, cross toolchain, NSS caveat
13. [Code shape and deferred decisions](#13-code-shape-and-deferred-decisions) — one crate, thin main, version scheme, `panic = "abort"`
14. [Bash hazards](#14-bash-hazards) — interpreted sinks, isolation footguns, TSV framing, GNU/BSD, `set -e`

---

## 1. Philosophy and Rules, as written before the rewrite
...
## 4. How it works, end, archive and purge
...
The conversation files ae deletes on `--purge-history` are the ones it can name by EXACT id:
claude's per-conversation transcript, codex's rollout, and agy's `<id>.db` with its `-wal`/`-shm`
siblings. gemini and opencode are named in a keep-note instead, because ae has no exact lookup
for them. A seat whose id was never captured names nothing and deletes nothing.

**A recorded harness session id is a NAME, never a path.** `harness_session.<slot>` is
hand-editable metadata and `set-harness-session` screens only for control bytes, so the purge
proves every id against the archive's UUID grammar BEFORE it builds a path — `Path::join` with
an absolute operand DISCARDS the store it was joined onto, and `../../x` walks out of it, which
turns a roster row into a `remove_file` target anywhere the user can write. The guard sits at the
row rather than in each tool's arm, so a fourth tool cannot be added without it, and an id that is
not a UUID is reported as a conversation left in place.
...
## 5. Session helpers: generation, argv[0] dispatch, `_run`
...
## 6. Agent tool capabilities: key constraints
...
| Capability | Claude Code | Codex | Gemini CLI | Antigravity CLI (`agy`) | Grok Build | OpenCode |
|---|---|---|---|---|---|---|
| **System prompt injection** | `--append-system-prompt 'text'` | `-c developer_instructions='text'` | `-i 'text'` | None (append-style) — `agy --help` (1.1.25, measured 2026-09-04) lists NO system-prompt flag at all, so context rides `-i`/`--prompt-interactive` as a USER TURN, gemini-shaped down to the wait suffix (measured: the seat answers "Understood… standing by" and does not act) | None (append-style) — context rides the POSITIONAL `[PROMPT]` argv. `--system-prompt-override` (and its 0.2.118+ compat alias `--system-prompt`) REPLACES grok's own agent prompt (breaks its tooling) — never use either | `OPENCODE_CONFIG=<per-slot json>` whose `instructions` array is loaded as system-level content (merges with the operator's config; arrays concatenate) |
| **Session ID at launch** | `--session-id UUID` (set by ae) | None (no flag exists) | None (launch token only; no launch-time UUID flag) | None (no flag exists — `--conversation` RESUMES, it does not create) | `--session-id UUID` (set by ae) | None (no flag exists) |
| **Session ID capture** | Immediate (ae generates UUID upfront) | Post-launch, four links tried in order: the id file codex's own first task writes, a launch-token scan of `~/.codex/sessions/<day>/*.jsonl`, a cwd scan of the same files, then the TUI header | Post-launch via local chat history scan (`~/.gemini/tmp/.../chats/session-*.json`) | Post-launch, two ALTERNATIVES chosen by whether the seat has a launch token — never a chain (measured 2026-09-04). With a token: searched in the BYTES of `~/.gemini/antigravity-cli/conversations/<id>.db` (one SQLite file per conversation, the file stem IS the id), bounded — streamed in chunks, first match wins, and skipped when the node is not a regular file (a FIFO named `<uuid>.db` reports length 0 and would BLOCK the watchdog on `open(2)`) or when it exceeds 16 MiB, measured both by the stat AND by a cumulative read budget, because a live database can grow after it is measured. Without one (legacy meta): agy's own per-process log `~/.gemini/antigravity-cli/log/cli-*.log`, which carries `workspaceDirs=[…]` and `Created conversation <id>`, and ONLY when exactly one conversation matches. TWO stores because agy splits the facts: the database holds the token but never the working directory, the log holds the directory but never the token. **A token miss stays `pending`** — falling through to the workspace cross-wired two seats sharing one directory (reviewer repro, fixed before merge) | Immediate (ae generates UUID upfront) — no launch token, no `_register-sid` handshake | Post-launch via `opencode session list --format json` (directory + launch-time match). The launch-token DB scan is inert: it looks for `AE_OPENCODE_LAUNCH_ID` in a message part, and nothing is pasted any more (measured: instructions content does not reach message parts) |
| **Resume with exact session** | `--resume UUID` | `codex <flags> resume UUID` (`resume` is a subcommand) | `--resume UUID` on current CLI; `ae` falls back to `--resume latest` when uncaptured | `--conversation UUID` (verified 2026-09-04: transcript restored, and it composes with `-i` so ae re-injects context on the same line) | `--resume UUID` | `--session ID` (e.g. `ses_...`) |
| **Resume fallback** | `--continue` (CWD heuristic) | Fresh start (drop `resume UUID`, keep flags) | `--resume latest` | `--continue` (most recent conversation). A `--conversation` id agy does not know warns `conversation "…" not found` and starts FRESH rather than aborting (measured), but ae does not rely on that: it probes for `<id>.db` first | `--continue` (CWD heuristic) | `--continue` (last session) |
| **Concurrent session safety** | Full — UUID-scoped | Partial — the self-registration handshake and launch tokens reduce collisions, but fallback CWD matching is still heuristic | Partial — UUID-scoped once captured; fallback `--resume latest` remains heuristic when uncaptured | Full once captured, and safe while it is not: the token is the only positive proof for a seat that has one, so two agy seats in one directory cannot take each other's conversation — they stay `pending` until each token lands. The workspace fallback exists only for a seat with NO token, and refuses to choose between two candidates | Full — UUID-scoped | Partial — `--session ID` is UUID-scoped once captured; fallback CWD matching remains heuristic |
| **Config flags preserved on resume** | Yes (flags stay, `--resume` appended) | Yes (flags before `resume` subcommand) | Yes (flags stay, `--resume` appended) | Yes, with the operator's own session surface stripped first: `--conversation`, `--continue` and `-c` are removed by ae's agy-aware stripper so nothing stacks with the id ae is putting on | Yes (flags stay, `--resume`/`--continue` appended) | Yes (flags stay, `--session` appended) |
| **TUI modelled for delivery** | Yes — input-busy + staged detection; bracketed paste measured 2026-08-30 (plain head loss 4/4, bracketed 0/6, receiver byte-exact) | Yes — busy detection (staged detection: see input-region work) | No | No | No (v1) — sends deliver unprotected: no typed-input protection, no staged detection, no throttle patterns until its TUI is observed live | No |
| **Launch-time input-ready detection** | Idle-input sensor; no separate start-up marker observed | `model: loading` header **and** `Starting MCP servers (n/m)` progress line are NOT-ready markers (measured 2026-08-15, v0.147.0: input box drawn at t=0.5s, settled t=3.0s) | None — falls back to the composed-UI grep, but nothing rides on it: context arrives via OPENCODE_CONFIG, not a paste | None, and nothing rides on it: context arrives on the argv, not a paste. But see the TRUST MODAL below — a workspace agy does not trust blocks the pane on a dialog before any input is read | **EXEMPT — no readiness detection.** Its TUI is unmodelled, so launch delivery is ungated and a slow start can land a paste into an unready pane. Accepted risk, not a pretend gate | None — falls back to the composed-UI grep |
| **`_run` re-run** | Resumes — first run creates, later runs exec the `--resume` variant when the conversation is still there and the tool's own fallback when it is not | Starts FRESH (no id to bake, so nothing collides — the conversation is simply lost) | Starts FRESH | Resumes once the capture has answered — verified live 2026-09-04 in `ae-dev`: a respawned pane command came back on the same conversation, same `<id>.db`, prior turns intact. `--continue` when the id is missing or its `<id>.db` is gone | Resumes — same as claude | Starts FRESH |
...
- **agy's launch is gated by a TRUST MODAL, and the trust list is EXACT-PATH.** A workspace
  not listed in `trustedWorkspaces` (agy's `~/.gemini/antigravity-cli/settings.json`) draws
  "Do you trust the contents of this project?" and the pane sits on it until a human answers
  — measured 2026-09-04, and measured again for the prefix question: `/Users/ckriech` being
  trusted does NOT cover `/Users/ckriech/projects/clemens33/agy-live-probe`, so every fresh
  work directory prompts once. ae's context survives it, because it rides the ARGV and agy
  replays it after the dialog (verified: the full workspace context and identity line
  arrived after a single Enter). A pasted BRIEF would not — Enter answers the dialog. Same
  class as claude's trust dialog, and the reason `_spawn_input_ready` exists; agy has no
  readiness predicate, so a spawn into an untrusted directory is the one delivery ae cannot
  protect. The fix is the operator's, not ae's: trust the directory, or spawn agy only into
  one already trusted.
- tmux 3.4 escapes control characters in `-F` / `display-message` output as octal (`\x1f` returns as the literal `\037`, TAB as `_`); tmux 3.5+ emits them raw. EVERY ae format therefore uses the printable ` | ` separator (`tmux::WATCH_PANE_SEPARATOR`) with the free-text command field last, read with a bounded split; a test pins all twelve format constants control-character-free. Measured 2026-08-30 for the watchdog, and again 2026-09-04 when seven other formats still carried `\x1f`: on ubuntu-24.04 (tmux 3.4) the dead-pane probe went blind, so a send to a dead agent waited out the 30s busy bound instead of refusing, and the session-id resolver answered None, so `telegram stop` reported stopped without killing the bridge — invisible on macOS, where tmux is 3.5+.
...
- Meta v2 (core-written) carries the roster as `seat.<slot>=<name>`, `profile.<slot>=<profile>`, `agent_bin.<slot>=<binary>` and `harness_session.<slot>=<id>`; the legacy `agent.<slot>=alias:name:session_id` row is NEITHER written nor read. Human ruling 2026-09-04: a meta carrying one is a session to start over from, not one to migrate. The row is RECORDED as a loss anomaly rather than dropped — `ae list` shows such a session with an empty `agents[]` and `degraded: true`, so a legacy session never reads like a healthy one whose roster is simply empty — and every write path and `_roster list` refuse it, naming the session and the rule. `_roster <dir> migrate` is gone with it. The one reader that still parses `agent.<slot>` is `src/archive.rs`, deliberately: it is the frozen archive reader over inert data, and `ae <new> --from <uuid>` must keep working for archives written by the v1 line. Agent names must not contain `:` — one colon in a target means `<session>:<name>`.
...
## 8. Toolchain and lanes: the reasoning
...
## 10. Lint policy, tests and supply chain
...
## 11. Dependency posture (2026-08-24)
...
## 12. Fresh-clone reproducibility and the musl target
...
## 13. Code shape and deferred decisions
...
| JSON emitters (`events.jsonl`, `list --json`) | JSON syntax | core-owned, whole (`src/json.rs` renders, every control character escaped at write time). The surviving bash emits no JSON at all: the helpers' event append moved with the helpers, and `_event_json_str` went with it |
