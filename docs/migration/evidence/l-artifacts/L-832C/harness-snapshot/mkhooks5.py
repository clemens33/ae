...
# ─── v5 addition: the SINGULAR stop supervisor's entry ───────────────────────
# 27. the detached self-stop worker, at its own entry, so it can be caught ALIVE
#     without racing a process-table sampler.
out = insert_after(out,
    '_cmd_stop_supervisor() {\n    local name="$1"\n',
    '    _H b_stop_supervisor_singular_entry\n', 'b_stop_supervisor_singular_entry')

open(DST, 'w', encoding='utf-8').write(out)
print("v5 instrumented sha256:", hashlib.sha256(out.encode()).hexdigest())
