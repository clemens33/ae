...
Session: same
Origin: /private/tmp/aelx/L-FROM/name-never-infers/w
Directory: /private/tmp/aelx/L-FROM/name-never-infers/w
Mode: local
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/send "<agent_name>" "your message"
```

Ask another agent a question and require a reply:
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/ask <agent_name> "your question"
```

Request a critical review:
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/review <agent_name> "review request"
```

Reply to a logged request by request id:
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/reply <request_id> "your reply"
```

Inspect request state without peeking panes:
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/requests
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/requests inbox
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/state working "starting on X"
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/state waiting-user "need clarification on Y"
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/state blocked "waiting for codex review on req-Z"   # reason required
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/state done "shipped X"
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/state                                                # print current state
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/memo add "important shared fact"
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/memo add --topic arch "we chose SQLite over Postgres"
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/memo read
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/memo read --topic arch
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/memo tail 5
```

Memo entries are append-only and stored in `/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/memo.tsv`. Use memo for durable shared context, not full chat transcripts.
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/goal                       # show the current session goal
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/goal "ship the PR for X"   # set it
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/goal --clear               # remove it
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/send "<their_agent_name>" "your reply"
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/say "done — the menu is live, tested end to end"
echo "a longer, multi-line reply" | /tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/say   # stdin for long text
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/peek <agent_name> [lines]
```
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/peak <agent_name> [lines]
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/agents
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/focus <agent_name>
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/interrupt <agent_name> [message]
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/spawn <alias>:<name> [prompt]
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/retire <agent_name|pane_id>
```
...
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/peek @other-session:claude:lead 20
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/send @other-session:codex:reviewer "check the API"
```

List all agents across all running sessions:
```bash
/tmp/aelx/L-FROM/name-never-infers/h/.ae/sessions/same/agents --all
```
