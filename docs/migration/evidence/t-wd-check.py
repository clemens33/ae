...
def count_block(L):
    """Locate the generated counts block. Counts may appear ONLY here — that is a rule
    about PROVENANCE, not spelling, so a representation nobody enumerated is invalid by
    construction rather than by recognition. The adjacency check below is a belt over
