...
> **SUPERSEDED 2026-08-21 — the stored column is retired.** Verdict is now DERIVED from
> `OBLIGATIONS.tsv` rather than stored beside it: a stored copy is the shape that went stale, and
> we removed the possibility rather than policing it. This section is kept as the record of the
> PRE-REGISTRATION, which is a decision and not a derived artifact. `VERDICTS.tsv`, `verdicts.py`
> and `verify-verdicts.py` are recoverable from `da61fa5b`; see `VERDICTS-RECONCILIATION.md`.
