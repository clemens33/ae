...
bad, checked = [], 0
# Arm directories live under `arms/` in Batch C and directly under the tree as `A-*` in
# Batch H. Discovering both means a tree whose layout differs is CHECKED rather than
# silently reporting cases_checked=0 — a gate that audits nothing and says PASS.
arms = os.path.join(TREE, "arms")
arm_dirs = []
if os.path.isdir(arms):
    arm_dirs += [(a, os.path.join(arms, a)) for a in sorted(os.listdir(arms))]
H_ARMS = set()
for a in sorted(os.listdir(TREE)):
    if a.startswith("A-") and os.path.isdir(os.path.join(TREE, a)):
        arm_dirs.append((a, os.path.join(TREE, a))); H_ARMS.add(a)
for arm, adir in arm_dirs:
    ...
    for c in cases:
        ...
        # The required-artifact set is per BATCH LAYOUT. Batch C's `any` names its own
        # capture shape (case.txt, consumers.tsv, env-tab-selfcheck.txt, a manifest pair);
        # Batch H's cases have a different one. Applying C's set to H would either fail
        # every H case or force C's rules to be weakened to fit — so the kinds fork here
        # rather than the rules bending.
        if arm in H_ARMS:
            kinds = ["h-any"]
            if "event=surface-state-captured" in text: kinds.append("surface-state")
            if "event=invocation" in text: kinds.append("h-invocation")
            if text.count("event=canary") >= 2: kinds.append("canaried")
            for k in kinds:
                for f in req.get(k, []):
                    if f.startswith("glob:"):
                        _, n, pat = f.split(":", 2)
                        import glob as _g
                        if len(_g.glob(os.path.join(cdir, pat))) < int(n):
                            bad.append((f"{arm}/{c}", f, f"kind '{k}' requires at least {n} matching {pat}"))
                        continue
                    q = os.path.join(cdir, f.rstrip("/"))
                    ok = os.path.isdir(q) and bool(os.listdir(q)) if f.endswith("/") else os.path.exists(q)
                    if not ok:
        ...
        if any(e in text for e in ("event=consumer-START", "event=plain-consumer-START",
                                   ...
                                   "event=inside-invocation-COMPLETE")):
        ...
        # A MUTATING case is one whose measured invocation was allowed to write. Its
        # instruments therefore need their own controls, and the schema demands the
        # control READING, not merely the file that would carry it.
        if "event=witness-control-FIRED" in text: kinds.append("mutating")
        # An ABSENCE row is only readable beside the SOURCE state: a case that declares
        # it captured one must publish it, or "nothing rendered" has no companion fact.
        if "event=source-state-captured" in text: kinds.append("source-state")
        # Batch H cases: one capture wrapper, bounded invocations, canaries either side.
        if "event=surface-state-captured" in text: kinds.append("surface-state")
        if "event=invocation" in text: kinds.append("h-invocation")
        if text.count("event=canary") >= 2: kinds.append("canaried")
        ...
        for k in kinds:
            for f in req.get(k, []):
                if f.startswith("glob:"):
                    ...
                    continue
                if f.startswith("contains:"):
                    _, fname, lit = f.split(":", 2)
                    fp = os.path.join(cdir, fname)
                    body = open(fp, encoding="utf-8", errors="replace").read() if os.path.exists(fp) else ""
                    if lit not in body:
                        bad.append((f"{arm}/{c}", f, f"kind '{k}' requires {fname} to contain {lit!r}"))
                    continue
                ...
                if not ok:
                    bad.append((f"{arm}/{c}", f, f"required by kind '{k}' but absent or empty"))
        checked += 1
        base = re.sub(r"-(controlled-path|fixed-clock|ro-noserver|rw-noserver|ro|rw|live|twin|barrier|attach|follow|mutating)$", "", c)
        seen.add(base)
