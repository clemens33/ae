...
build_live_topology() { # <ae-home> <sock> <session>...
    # The roster comes from the fixture's own meta. Some fixtures make that file absent or
    # unreadable ON PURPOSE (A9's SC-405i and G3/meta-mode-000), and the old body simply
    # `continue`d or read nothing — the case then declared live_topology=yes while no
    # server existed at all, and the consumers ran against a socket that was never created.
    # AE_ROSTER_META names a substitute roster for exactly those fixtures; the substitution
    # is a controller action and is recorded with the roster's hash. Either way the built
    # topology is VERIFIED against the roster before the case proceeds: an instrument that
    # cannot report its own failure is not an instrument.
    ...
    for s in "$@"; do
        ...
        local roster="${AE_ROSTER_META:-$meta}"
        if [[ ! -r "$meta" ]]; then
            if [[ "$roster" == "$meta" ]]; then
                led HARNESS-ABORT "reason=this fixture's meta is absent or unreadable and no AE_ROSTER_META substitute was named" \
                    "session=$s" "meta=$meta"
                return 1
            fi
            led topology-roster-SUBSTITUTED "session=$s" \
                "reason=this fixture's own meta is absent or unreadable BY DESIGN, so the roster cannot come from it" \
                "roster_source=$roster" "roster_sha256=$(sha "$roster")" \
                "note=the substitute is the member this fixture was derived from; the topology is therefore the same across the arm and only the fixture's files differ"
        fi
        [[ -r "$roster" ]] || { led HARNESS-ABORT "reason=roster unreadable" "roster=$roster"; return 1; }
        wd="$(grep '^work_dir=' "$roster" | cut -d= -f2-)"; [[ -d "$wd" ]] || wd="$aehome"
        while IFS='=' read -r k v; do
            [[ "$k" == agent.* ]] || continue
            slot="${k#agent.}"; ref="${v%:*}"; want=$((want + 1))
            if ((first)); then
        ...
        command tmux -S "$sock" set-environment -t "$s" AE_ORIGIN "$(grep '^origin=' "$roster" | cut -d= -f2-)"
        command tmux -S "$sock" set-environment -t "$s" AE_DIR "$wd"
        command tmux -S "$sock" set-environment -t "$s" AE_MODE "$(grep '^mode=' "$roster" | cut -d= -f2-)"
        command tmux -S "$sock" set-environment -t "$s" AE_HOME "$aehome"
    done
    sleep 1
    local got sessions
    got="$(command tmux -S "$sock" list-panes -a -F x 2>/dev/null | wc -l | tr -d ' ')"
    sessions="$(command tmux -S "$sock" list-sessions -F '#{session_name}' 2>/dev/null | tr '\n' ',' | sed 's/,$//')"
    led live-topology-built "sessions=$*" "agent_binary=$FAKE_BIN" "socket=$sock" \
        ...
        "panes_expected=$want" "panes_present=$got" "sessions_present=${sessions:-<none>}"
    if [[ -z "$sessions" || "$got" != "$want" ]]; then
        led HARNESS-ABORT "reason=the live topology did not come up as declared" \
            "panes_expected=$want" "panes_present=$got" "sessions_present=${sessions:-<none>}"
        return 1
    fi
    return 0
