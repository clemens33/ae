...
int main(int argc, char **argv) {
    ...
    /* Per-pane control FIFO: with AEFAKE_CTL_DIR set, each instance creates and owns
       "<dir>/<TMUX_PANE>.ctl", so several fake agents in one session are independently
       drivable. AEFAKE_CTL (a single shared path) still works and takes precedence. */
    char ctlbuf[4096];
    const char *ctldir = getenv("AEFAKE_CTL_DIR");
    const char *pane = getenv("TMUX_PANE");
    if ((!ctlp || !*ctlp) && ctldir && *ctldir && pane && *pane) {
        snprintf(ctlbuf, sizeof ctlbuf, "%s/%s.ctl", ctldir, pane);
        unlink(ctlbuf);
        if (mkfifo(ctlbuf, 0600) == 0 || errno == EEXIST) ctlp = ctlbuf;
        if (lg) { fprintf(lg, "ctl=%s\n", ctlbuf); fflush(lg); }
    }
    int ctl = -1;
