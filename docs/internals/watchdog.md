...
flowchart TD
    ...
    Done -- yes --> SkipDone[skip — honor quiet<br/>done: event-only<br/>waits: until the human's input in the pane<br/>waiting-agent/blocked: proof challenge per cadence, same-state re-declare<br/>lapsed: verdict kept, nudge budget resumes<br/>waiting-agent past ceiling: as blocked]
