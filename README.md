...
- **A status bar that answers "who needs me"** -- inside its sessions ae draws the tmux footer: this session, its windows and mark-first agents (`●lead ✓colead ◌builder ⚠grok`), its branch and goal on line one; every ae session on the server, in your own `[workspace] fleet_order` and each one clickable, on line two. Verdicts, never claims, and `[workspace] theme = off` gives your own status line back.
